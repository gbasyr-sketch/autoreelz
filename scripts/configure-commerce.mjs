...
const labels={id:'ID',number:'Номер заказа',kind:'Вид заказа',status:'Состояние',payment_status:'Оплата',delivery_status:'Доставка',customer_name:'Покупатель',customer_phone:'Телефон',customer_email:'Email',delivery_snapshot:'Адрес и способ доставки',items_snapshot:'Неизменяемый состав заказа',product_total_kopecks:'Товары, копейки',shipping_cost_kopecks:'Доставка, копейки',shipping_reason:'Расчёт доставки',shipping_version:'Версия расчёта',shipping_package:'Упаковка',terms:'Срок и условия',review_reason:'Причина проверки',expires_at:'Оплатить до',created_at:'Создано',updated_at:'Обновлено',order_id:'ID заказа',amount_kopecks:'Сумма, копейки',currency:'Валюта',method:'Способ оплаты',sku_id:'Физический SKU',stock_delta:'Изменение физического остатка',reserved_delta:'Изменение резерва',reason:'Причина',actor_id:'ID сотрудника',event_type:'Событие',note:'Примечание',quantity:'Количество',name:'Название',package_id:'Упаковка',active:'Активно',components:'Точный состав (SKU → количество)',is_demo:'Демонстрационные данные'};
...
for(const [collection,[name,icon,template]]of Object.entries(groups)){
 ...
 for(const field of fields){if(field.type==='alias')continue;const key=field.field;const meta={interface:field.type==='json'?'input-code':field.type==='boolean'?'boolean':'input',options:field.type==='json'?{language:'json'}:{},sort:position++,width:field.type==='json'?'full':'half',hidden:['id','batch_id','session_id','allocation_state','operation_key'].includes(key),readonly:collection!=='ar_packing_rules'||key==='id',translations:[{language:'ru-RU',translation:labels[key]??key}]};
  ...
  if(['created_at','updated_at','expires_at'].includes(key))meta.display='datetime';
...
for(const[collection,fields]of Object.entries({ar_orders:['number','kind','status','customer_name','product_total_kopecks','created_at'],ar_stock_movements:['sku_id','stock_delta','reserved_delta','reason','created_at'],ar_payments:['order_id','amount_kopecks','method','status','created_at']})){
