...
export const products: DemoProduct[] = [
  {id:id(100),slug:'heater-control',name:'Блок управления отопителем',category:'heaters',kind:'single',description,image:'/images/placeholder-console.svg',variants:[
    {id:id(1001),article:'DEMO-HEATER-R',label:'Красная подсветка',priceKopecks:490000,stock:7,attributes:{illumination:'red',surface:'matte',material:'demo-plastic'}},
    {id:id(1002),article:'DEMO-HEATER-B',label:'Синяя подсветка',priceKopecks:510000,stock:3,attributes:{illumination:'blue',surface:'matte',material:'demo-plastic'}},
    {id:id(1003),article:'DEMO-HEATER-W',label:'Белая подсветка',priceKopecks:530000,stock:0,attributes:{illumination:'white',surface:'gloss',material:'demo-plastic'}},
  ]},
  {id:id(101),slug:'interior-console',name:'Автомобильная консоль',category:'consoles',kind:'single',description,image:'/images/placeholder-interior.svg',variants:[
    {id:id(1011),article:'DEMO-CONSOLE-M',label:'Матовая поверхность',priceKopecks:690000,stock:4,attributes:{surface:'matte',material:'demo-plastic'}},
    {id:id(1012),article:'DEMO-CONSOLE-G',label:'Глянцевая поверхность',priceKopecks:720000,stock:0,attributes:{surface:'gloss',material:'demo-plastic'}},
  ]},
  {id:id(102),slug:'amg-vent',name:'Воздуховод AMG с подсветкой',category:'vents',kind:'single',description,image:'/images/placeholder-vent.svg',variants:[
    {id:id(1021),article:'DEMO-VENT-R',label:'Красная подсветка',priceKopecks:240000,stock:12,attributes:{illumination:'red',surface:'matte'}},
    {id:id(1022),article:'DEMO-VENT-RGB',label:'RGB-подсветка',priceKopecks:320000,stock:2,attributes:{illumination:'rgb',surface:'matte'}},
  ]},
  {id:id(103),slug:'android-radio',name:'Android-магнитола',category:'radios',kind:'single',description,image:'/images/placeholder-radio.svg',variants:[
    {id:id(1031),article:'DEMO-RADIO',label:'Базовое исполнение',priceKopecks:1490000,stock:0,attributes:{}},
  ]},
  {id:id(104),slug:'gear-knob',name:'Ручка переключения передач',category:'knobs',kind:'single',description,image:'/images/placeholder-knob.svg',variants:[
    {id:id(1041),article:'DEMO-KNOB',label:'Матовое исполнение',priceKopecks:180000,stock:8,attributes:{surface:'matte',material:'demo-metal'}},
  ]},
  {id:id(105),slug:'radio-frame',name:'Рамка под Android-магнитолу',category:'radio-frames',kind:'single',description,image:'/images/placeholder-frame.svg',variants:[
    {id:id(1051),article:'DEMO-FRAME',label:'Матовая поверхность',priceKopecks:150000,stock:6,attributes:{surface:'matte',material:'demo-plastic'}},
  ]},
...
export interface Offer {priceKopecks:number;available:number;variant?:DemoVariant}
export function offerFor(product:DemoProduct, skuId?:string|null):Offer|null {
  if (product.kind==='bundle') {
    ...
    let sum=0,available=Infinity;
    for(const [skuId,quantity] of required){const found=findSku(skuId);if(!found)return null;sum+=found.variant.priceKopecks*quantity;available=Math.min(available,Math.floor(found.variant.stock/quantity));}
    return {priceKopecks:Math.floor((sum*(10000-(product.discountBps??0))+5000)/10000),available};
  }
  const variant=skuId?product.variants.find(v=>v.id===skuId):product.variants[0];
  return variant?{variant,priceKopecks:variant.priceKopecks,available:variant.stock}:null;
}
