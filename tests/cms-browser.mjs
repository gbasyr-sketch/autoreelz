...
for(const role of ['admin','editor']){
 ...
 const article=`QA-${role.toUpperCase()}-${stamp}`;const sku=await create(p,'ar_skus',async()=>{await relation(p,'product_id',productName);await fill(p,'article',article);await fill(p,'name','Проверка исполнения');await fill(p,'price_rubles','1250.00');});
 await p.goto(base+'/admin/content/ar_skus/'+sku.id,{waitUntil:'networkidle'});await waitValue(p,'article',article);if(await field(p,'article').locator('input').inputValue()!==article)throw new Error('SKU reopen failed');if(Number(await field(p,'price_rubles').locator('input').inputValue())!==1250)throw new Error('SKU price did not preserve 1250.00 rubles');await p.screenshot({path:`artifacts/stage-3/${role}-sku-created.png`,fullPage:true});
