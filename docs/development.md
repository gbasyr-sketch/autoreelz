...
Основной адрес этапа 4: http://127.0.0.1:14323/. Каталог читается из новой CMS, корзина и заказы сохраняются в PostgreSQL. Оплата, доставка и письма работают только в локальном тестовом режиме.
...
`src/lib/demo.ts` сохранён для изолированных тестов и истории этапа 2; рабочая витрина его не импортирует. Избранное пока хранится в браузере, его синхронизация относится к этапу 5. Архив прототипов — порт 14321.
