...
> Базовый контракт этапа5. Указанные агенты относятся к тогдашнему распределению работ и не дают поручения запускать субагентов. Актуальная карта модулей — [code-map.md](code-map.md), состояние — [status.md](status.md).
