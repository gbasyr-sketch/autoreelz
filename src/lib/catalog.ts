...
export function queryCatalog(params:URLSearchParams,catalog:CatalogSnapshot){
 ...
 const price=(name:string)=>{const raw=params.get(name);if(!raw)return undefined;if(!/^\d{1,7}(?:[.,]\d{1,2})?$/.test(raw)){error='Укажите цену числом от 0 до 9 999 999 ₽, не более двух знаков после запятой.';return undefined;}return Math.round(Number(raw.replace(',','.'))*100);};
 ...
 if(min!==undefined)activeFilters.push({key:'min',label:`От ${min/100} ₽`});if(max!==undefined)activeFilters.push({key:'max',label:`До ${max/100} ₽`});
 ...
 const matches=(product:CatalogProduct,offer:Offer)=>{
  if(min!==undefined&&offer.priceKopecks<min)return false;if(max!==undefined&&offer.priceKopecks>max)return false;
 ...
 for(const product of products){
  ...
  const matching=offers.flatMap(offer=>{const fitment=selection?catalog.fitmentFor(product,offer,selection):undefined;if(fitment==='unknown')unknownFitment=true;if(fitment==='incompatible'||(fitment==='unknown'&&!includeUnknown))return[];return[{product,offer,fitment}];}).sort((a,b)=>a.offer.priceKopecks-b.offer.priceKopecks);
 ...
 if(sort==='price-asc')items.sort((a,b)=>a.offer.priceKopecks-b.offer.priceKopecks);if(sort==='price-desc')items.sort((a,b)=>b.offer.priceKopecks-a.offer.priceKopecks);if(sort==='name')items.sort((a,b)=>a.product.name.localeCompare(b.product.name,'ru'));
