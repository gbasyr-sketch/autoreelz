...
flowchart LR
  ...
  Worker --> Yoo
  Browser --> Maps[Яндекс.Карты после согласия]
```
...
Yandex SDK подключается после записи отдельного согласия; сами пункты/тарифы даёт СДЭК. Список сохраняет работоспособность при ошибке карты. [Контракт СДЭК](cdek-checkout-contract.md).
