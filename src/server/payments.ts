...
export async function applyPaymentEvent(input:PaymentEvent,signature:string){
 ...
 const paymentId=uuid(input.paymentId),eventId=text(input.eventId,'Событие',1,160),amount=integer(input.amountKopecks,'Сумма');
 ...
 return transaction(async c=>{
  ...
  }else if(input.currency!=='RUB'||amount!==safeMoney(current.amount_kopecks)||amount!==safeMoney(BigInt(order.product_total_kopecks)+BigInt(order.shipping_cost_kopecks??0))||order.shipping_version!==current.shipping_version){
...
export async function initiatePayment(session:ShopSession,body:Record<string,unknown>){
 ...
 const result=await transaction(c=>idempotent(c,`pay:${session.id}`,body.idempotencyKey,{id,version,method},async()=>{
  ...
  if(order.delivery_status!=='quoted'||order.shipping_cost_kopecks===null)throw new StoreError('DELIVERY_PENDING','Сначала менеджер должен уточнить доставку.',409);
  ...
  if(existing)return{orderId:id,paymentId:existing.id,amountKopecks:safeMoney(existing.amount_kopecks)};
  const amount=safeMoney(BigInt(order.product_total_kopecks)+BigInt(order.shipping_cost_kopecks));
  const payment=(await c.query('INSERT INTO ar_payments(order_id,amount_kopecks,method,shipping_version) VALUES($1,$2,$3,$4) RETURNING id',[id,amount,method,version])).rows[0];
  await c.query("UPDATE ar_orders SET payment_status='pending' WHERE id=$1",[id]);return{orderId:id,paymentId:payment.id,amountKopecks:amount};
 }));
...
export async function payOrder(session:ShopSession,body:Record<string,unknown>){
 ...
 if('paymentId'in payment&&payment.paymentId){const{event,signature}=testPaymentAdapter.event({id:payment.paymentId,amountKopecks:payment.amountKopecks!});await applyPaymentEvent(event,signature);}
