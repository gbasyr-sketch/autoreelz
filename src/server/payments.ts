...
export async function initiatePayment(session:ShopSession,body:Record<string,unknown>){
 ...
 const result=await transaction(c=>idempotent(c,`pay:${session.id}`,body.idempotencyKey,{id,version,method},async()=>{
  ...
  if(existing)return{orderId:id,paymentId:existing.id,amountRubles:safeMoney(existing.amount_rubles)};
  const amount=sumRubles([order.product_total_rubles,order.shipping_cost_rubles]);
  const payment=(await c.query('INSERT INTO ar_payments(order_id,amount_rubles,method,shipping_version) VALUES($1,$2,$3,$4) RETURNING id',[id,amount,method,version])).rows[0];
...
export async function payOrder(session:ShopSession,body:Record<string,unknown>){
 ...
 if('paymentId'in payment&&payment.paymentId){const{event,signature}=testPaymentAdapter.event({id:payment.paymentId,amountRubles:payment.amountRubles!});await applyPaymentEvent(event,signature);}
