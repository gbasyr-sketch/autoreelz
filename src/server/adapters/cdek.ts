...
export function createCdekClient(config:{clientId:string;clientSecret:string},transport:CdekTransport=fetch){
 ...
 async function directory(path:string){
  ...
  const promise=request(path).then(value=>{if(!Array.isArray(value))return fail('INVALID_RESPONSE');if(cache.size>=100)cache.delete(cache.keys().next().value!);cache.set(path,{value,until:Date.now()+5*60_000});return value;});pending.set(path,promise);try{return await promise;}finally{pending.delete(path);}
 }
