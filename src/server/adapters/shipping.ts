...
export async function quoteLocalShipping(c:PoolClient,need:Map<string,number>,delivery:CheckoutInput['delivery'],fail=false):Promise<ShippingQuote>{
 requireLocalTest();
 const pending=(reason:string):ShippingQuote=>({status:'pending_quote',costKopecks:null,label:'Стоимость уточнит менеджер',reason,packageSnapshot:null});
 try{
  ...
  return{status:'quoted',costKopecks:delivery.method==='courier'?80000:50000,label:delivery.method==='courier'?'Тестовая доставка курьером':'Тестовая доставка до ПВЗ',reason:null,packageSnapshot:{id:pack.id,name:pack.name,weightG:pack.weight_g,lengthMm:pack.length_mm,widthMm:pack.width_mm,heightMm:pack.height_mm}};
