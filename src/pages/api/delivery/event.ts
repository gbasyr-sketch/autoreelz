...
import {appConfig,requireTestEnvironment} from '../../../server/config';
...
export const POST:APIRoute=async ctx=>{try{
 requireTestEnvironment();if(ctx.url.host!==new URL(appConfig().origin).host)throw new StoreError('ORIGIN','Источник запроса не разрешён.',403);
